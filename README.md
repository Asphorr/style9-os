...
Next on the roadmap: **directories that can be made and removed**.  The
volume's own are read, walked and written into, but `mkdir` exists
nowhere in this kernel -- not in the APFS writer, not in the Darwin
syscall table -- so a program can put a file wherever a directory already
is, and nowhere else.  Also open: a create that splits and retries the
way growing a file does, rename, and real SMP.
...
real Apple shell redirecting into a file, and nodes that leave the tree
when they empty were all on this list once and have since landed.)
