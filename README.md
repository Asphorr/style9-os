...
Next on the roadmap: **a file whose records need not share a leaf**.  It
is a real ceiling now rather than a theoretical one -- the test
container's tree has split often enough that `/var/db/big.txt` keeps its
runs in one leaf and its inode in another, so the truncate and grow
self-tests report the refusal and skip instead of cutting.  Also open: a
create that splits and retries the way growing a file does, a tree that
gives nodes BACK when they empty (it only ever gains them), directories
that can be made and removed, and real SMP.
...
virtual-copy OOL semantics, files that can be created and removed, and a
B-tree that grows a level were all on this list once and have since
landed.)
